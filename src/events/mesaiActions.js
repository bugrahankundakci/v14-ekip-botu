            ...
            switch (selectedValue) {
                ...
                case 'forcemesailer':
                    ...
                    const forcemesailerInput = new TextInputBuilder()
                        ...
                        .setLabel('Mesai kontrol edilecek kişi.')
                        .setPlaceholder('Mesai bilgileri alınacak kişinin Discord ID\'sini girin...')
                        .setStyle(TextInputStyle.Short);
            ...
            switch (modalId) {
                ...
                case 'forcemesailerModal':
                    ...
                    for (const [key, value] of Object.entries(userForceData.mesailer)) {
                        const girişZamanı = moment(value.giriş);
                        const çıkışZamanı = value.çıkış ? moment(value.çıkış) : moment();
                        const süre = moment.duration(çıkışZamanı.diff(girişZamanı));

                        const saat = Math.floor(süre.asHours());
                        const dakika = Math.floor(süre.asMinutes()) % 60;
                        const saniye = Math.floor(süre.asSeconds()) % 60;

                        fileContent += `**${key}. Mesai**\nGiriş: ${moment(value.giriş).format('YYYY-MM-DD HH:mm:ss')}\nÇıkış: ${value.çıkış ? moment(value.çıkış).format('YYYY-MM-DD HH:mm:ss') : 'Henüz çıkış yapılmamış'}\nSüre: ${saat} saat ${dakika} dakika ${saniye} saniye\n\n`;
                    }
                ...
                case 'forcemesaicheckModal':
                    ...
                    const lastIndex = Object.keys(mesailer).length;

                    if (lastIndex === 0) {
                        await interaction.reply({ content: `<@${forcemesaicheckInputValue}> kullanıcısının henüz hiçbir mesai kaydı yok.`, ephemeral: true });
                        return;
                    }

                    const lastMesai = mesailer[lastIndex];
                    const girişZamanı = moment(lastMesai.giriş);
                    const çıkışZamanı = lastMesai.çıkış ? moment(lastMesai.çıkış) : moment();
                    const süre = moment.duration(çıkışZamanı.diff(girişZamanı));
                    ...
                    const checkMessage = `Son Mesai Bilgisi:\nGiriş: <t:${Math.floor(new Date(lastMesai.giriş).getTime() / 1000)}:R>\nÇıkış: ${lastMesai.çıkış ? `<t:${Math.floor(new Date(lastMesai.çıkış).getTime() / 1000)}:R>` : 'Henüz çıkış yapılmamış'}\nSüre: ${saat} saat ${dakika} dakika ${saniye} saniye\n`;
